 ...
 * 
 * Custom page template for member login
 * 
...
// Check for membership status errors from redirect
if (isset($_GET['status_pending'])) {
  $login_errors[] = 'Your membership is pending approval. Please contact us for assistance.';
} elseif (isset($_GET['status_suspended'])) {
  $login_errors[] = 'Your membership has been suspended. Please contact us to resolve this issue.';
} elseif (isset($_GET['status_expired'])) {
  $login_errors[] = 'Your membership has expired. Please contact us to renew your membership.';
}
...
if (isset($_POST['member_login_submit'])) {
  if (!isset($_POST['member_login_nonce']) || !wp_verify_nonce($_POST['member_login_nonce'], 'member_login_action_-1')) {
    ...
    if (empty($login_errors)) {
      ...
      if (is_wp_error($user)) {
        ...
        if (Member::is_user_member($user->ID)) {
          ...
          if ($member_status !== 'active') {
            ...
            $login_errors[] = isset($status_messages[$member_status])
          ...
          // Not a member, log them out
          wp_logout();
          $login_errors[] = 'This login page is for members only. Please use the standard WordPress login.';
        }
      }
    }
  }
...
<div id="member-login" class="flex min-h-[70vh] items-center justify-center bg-background">
  <div class="w-full max-w-md mx-auto">
    <div class="card p-8 shadow-lg">
      <div class="text-center mb-8">
        <h1 class="text-3xl font-bold mb-2">Member Login</h1>
        <p class="text-muted-foreground">Sign in to access your member dashboard</p>
      </div>
      ...
      <!-- Login Form -->
      <form method="post" action="" class="form grid gap-6 member-login-form">
        <?php wp_nonce_field('member_login_action_-1', 'member_login_nonce'); ?>
        ...
        <button type="submit" name="member_login_submit" class="btn w-full">Sign In</button>

        <div class="text-center">
          <a href="<?php echo Member::get_forgot_password_url(); ?>"
    ...
    <div class="mt-6 text-center text-sm text-muted-foreground">
      <p>
        Not a member yet? <a href="<?php echo home_url('/contact'); ?>" class="text-blue-600 hover:underline">Contact
          us</a>
      </p>
      <p class="mt-2">
